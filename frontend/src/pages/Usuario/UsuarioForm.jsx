...
import { 
  ...
  FiRefreshCw,
  FiCheckCircle,
  FiCreditCard, // Usamos FiCreditCard en lugar de FiIdCard
  FiAlertCircle,
  FiEye,
  FiEyeOff
} from 'react-icons/fi';
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4 md:p-6">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-indigo-100 text-xs sm:text-sm truncate">
                  {usuarioEdit ? 'Modifica los datos del usuario' : 'Completa el formulario para registrar un nuevo usuario'}
                </p>
              </div>
            </div>
            <button
        ...
        {/* FORMULARIO */}
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-5 sm:space-y-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          {/* BOTONES */}
          <div className="flex flex-col sm:flex-row gap-3 pt-4 sm:pt-6 border-t border-gray-200 sticky bottom-0 bg-white pb-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 sm:px-6 py-3 border border-gray-300 text-gray-700 hover:bg-gray-50 active:bg-gray-100 rounded-xl font-medium transition-all duration-200 active:scale-[0.98]"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-4 sm:px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-700 hover:from-indigo-700 hover:to-purple-800 text-white rounded-xl font-medium flex items-center justify-center gap-2 transition-all duration-200 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
            >
              {loading ? (
                <>
